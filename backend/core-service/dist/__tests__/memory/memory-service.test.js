...
class MemoryService {
    l1Window = new Map();
    l1TimeoutMs;
    l1Timestamps = new Map();
    db;
    constructor(l1TimeoutMs = 5 * 60 * 1000) {
        this.l1TimeoutMs = l1TimeoutMs;
        this.db = db_1.pool;
    }
    // L1: Temporary Window
    async addL1Message(soulId, message) {
        const entry = {
            ...message,
            id: this.generateId(),
            timestamp: Date.now(),
        ...
        await this.db.query('INSERT INTO l1_messages (id, soul_id, role, content, timestamp, metadata) VALUES ($1, $2, $3, $4, $5, $6)', [entry.id, soulId, entry.role, entry.content, entry.timestamp, JSON.stringify(entry.metadata || {})]);
        return entry;
    }
    async getL1Window(soulId) {
    ...
    clearL1Window(soulId) {
        this.l1Window.delete(soulId);
        this.l1Timestamps.delete(soulId);
    }
    // L2: Daily Summary
    async createL2Summary(soulId, summary, topics) {
        ...
        await this.db.query('INSERT INTO l2_summaries (id, soul_id, date, summary, topics, created_at) VALUES ($1, $2, $3, $4, $5, $6)', [entry.id, soulId, date, summary, topics, entry.createdAt]);
        return entry;
    }
    async getL2Summaries(soulId, limit = 30) {
        const result = await this.db.query('SELECT * FROM l2_summaries WHERE soul_id = $1 ORDER BY date DESC LIMIT $2', [soulId, limit]);
        return result.rows || [];
    }
    async getL2SummaryByDate(soulId, date) {
        const result = await this.db.query('SELECT * FROM l2_summaries WHERE soul_id = $1 AND date = $2 LIMIT 1', [soulId, date]);
        return result.rows?.[0] || null;
    }
    // L3: Knowledge Base
    async updateL3Knowledge(soulId, topic, content, confidence = 1.0) {
        const existing = await this.db.query('SELECT id FROM l3_knowledge WHERE soul_id = $1 AND topic = $2 LIMIT 1', [soulId, topic]);
        const now = Date.now();
        if (existing.rows?.length > 0) {
            // Merge: update existing
            await this.db.query('UPDATE l3_knowledge SET content = $1, confidence = $2, updated_at = $3 WHERE soul_id = $4 AND topic = $5', [content, confidence, now, soulId, topic]);
            return {
            ...
            await this.db.query('INSERT INTO l3_knowledge (id, soul_id, topic, content, confidence, updated_at) VALUES ($1, $2, $3, $4, $5, $6)', [id, soulId, topic, content, confidence, now]);
            return { id, soulId, topic, content, confidence, updatedAt: now };
        }
    }
    async getL3Knowledge(soulId, topic) {
        if (topic) {
            const result = await this.db.query('SELECT * FROM l3_knowledge WHERE soul_id = $1 AND topic = $2 ORDER BY confidence DESC', [soulId, topic]);
            return result.rows || [];
        }
        const result = await this.db.query('SELECT * FROM l3_knowledge WHERE soul_id = $1 ORDER BY updated_at DESC', [soulId]);
        return result.rows || [];
    }
    async mergeL3Knowledge(soulId, newEntries) {
        let merged = 0;
...
describe('MemoryService', () => {
    ...
    describe('L1 Temporary Window', () => {
        ...
        it('should persist L1 message to database', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO l1_messages'), expect.arrayContaining([
                expect.any(String),
                'soul-1',
                'user',
                'Persist me',
    ...
    describe('L2 Daily Summary', () => {
        ...
        it('should persist L2 summary to database', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO l2_summaries'), expect.arrayContaining([
                expect.any(String),
                'soul-1',
                expect.any(String),
                'Summary text',
        ...
        it('should retrieve L2 summaries with limit', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('ORDER BY date DESC LIMIT'), ['soul-1', 30]);
        });
    ...
    describe('L3 Knowledge Base', () => {
        ...
        it('should update existing L3 knowledge entry', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE l3_knowledge'), expect.arrayContaining([
                'Updated content',
                0.98,
                expect.any(Number),
                'soul-1',
        ...
        it('should retrieve L3 knowledge by topic', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('AND topic ='), ['soul-1', 'API']);
        });
