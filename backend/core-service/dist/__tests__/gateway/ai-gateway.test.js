...
class AIGateway {
    ...
    loadProviders() {
        for (const provider of this.config.providers) {
            ...
            this.httpClients.set(provider.name, axios_1.default.create({
    ...
    async chatCompletion(provider, request) {
    ...
    async streamChat(provider, request) {
        const client = this.httpClients.get(provider);
        if (!client) {
            throw new Error(`Provider '${provider}' not found`);
        }
    ...
    resolveModel(provider, modelAlias) {
        const config = this.providers.get(provider);
        if (!config)
            throw new Error(`Provider '${provider}' not found`);
        if (modelAlias && config.models.includes(modelAlias))
            return modelAlias;
...
describe('AIGateway', () => {
    ...
    describe('Provider Loading', () => {
        ...
        it('should create axios instances for each provider', () => {
            ...
            expect(axios_1.default.create).toHaveBeenCalledTimes(3);
    ...
    describe('chatCompletion', () => {
        ...
        beforeEach(() => {
            ...
            axios_1.default.create.mockReturnValue(mockClient);
        });
...
//# sourceMappingURL=ai-gateway.test.js.map