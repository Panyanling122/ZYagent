...
class TokenRateLimiter {
    ...
    /** 获取单例实例 */
    static getInstance() {
        if (!TokenRateLimiter.instance)
            TokenRateLimiter.instance = new TokenRateLimiter();
        return TokenRateLimiter.instance;
    }
    /** 检查Token消耗是否超过日限额，超额返回false */
    async checkLimit(soulId, requestedTokens) {
        const today = new Date().toISOString().split("T")[0];
        const key = `${soulId}:${today}`;
        let used = this.cache.get(key);
        if (used === undefined) {
            const result = await this.db.query(`SELECT COALESCE(SUM(total_tokens), 0) as total FROM token_usage WHERE soul_id = $1 AND date = $2`, [soulId, today]);
            used = parseInt(result.rows[0].total);
            this.cache.set(key, used);
        }
        const soulResult = await this.db.query(`SELECT max_tokens_per_day FROM souls WHERE id = $1`, [soulId]);
    ...
    /** 记录本次Token消耗到缓存和数据库 */
    async recordUsage(soulId, model, promptTokens, completionTokens) {
        const today = new Date().toISOString().split("T")[0];
        const hour = new Date().getHours();
        const total = promptTokens + completionTokens;
        await this.db.query(`INSERT INTO token_usage (soul_id, model, prompt_tokens, completion_tokens, total_tokens, date, hour, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [soulId, model, promptTokens, completionTokens, total, today, hour]);
        const key = `${soulId}:${today}`;
...
exports.TokenRateLimiter = TokenRateLimiter;
