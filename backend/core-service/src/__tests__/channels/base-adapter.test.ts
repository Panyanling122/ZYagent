...
abstract class BaseChannelAdapter {
  ...
  abstract sendMessage(channelId: string, content: string, options?: SendOptions): Promise<{ messageId: string }>;
  ...
  protected validateConfig(): boolean {
    if (!this.config.channelType) return false;
    if (!this.config.enabled) return true; // Disabled configs don't need validation
    return true;
  }
...
class WeChatAdapter extends BaseChannelAdapter {
  ...
  async sendMessage(channelId: string, content: string, options?: SendOptions): Promise<{ messageId: string }> {
    if (!this.connected) {
  ...
  parseIncomingWebhook(payload: Record<string, unknown>): ChannelMessage | null {
    ...
    return {
      id: (payload.MsgId as string) || this.generateMessageId(),
      content: (payload.Content as string) || '',
      senderId: (payload.FromUserName as string) || '',
      senderName: (payload.FromUserName as string) || '',
      channelId: (payload.ToUserName as string) || '',
      timestamp: parseInt((payload.CreateTime as string) || '0') * 1000,
...
class FeishuAdapter extends BaseChannelAdapter {
  ...
  parseIncomingWebhook(payload: Record<string, unknown>): ChannelMessage | null {
    ...
    return {
      id: (message.message_id as string) || this.generateMessageId(),
      content: (message.content as string) || '',
      senderId: (sender?.sender_id?.union_id as string) || '',
      senderName: (sender?.sender_id?.union_id as string) || '',
      channelId: (message.chat_id as string) || '',
      timestamp: parseInt((message.create_time as string) || '0'),
      messageType: (message.message_type as ChannelMessage['messageType']) || 'text',
    };
  }
...
describe('BaseChannelAdapter', () => {
  const wechatConfig: ChannelConfig = {
    channelType: 'wechat',
    apiKey: 'wx_appid_123',
  ...
  const feishuConfig: ChannelConfig = {
    channelType: 'feishu',
    apiKey: 'fs_appid_123',
  ...
  describe('Webhook parsing', () => {
    ...
    it('should return null for invalid WeChat payload', () => {
      ...
      expect(adapter.parseIncomingWebhook(null as unknown as Record<string, unknown>)).toBeNull();
    });
