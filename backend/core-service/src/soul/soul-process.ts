...
class SoulProcessManager extends EventEmitter {
  private static instance: SoulProcessManager;
  private souls = new Map<string, ChildProcess>();
  ...
  async startSoul(soulId: string): Promise<void> {
    ...
    // 更新状态为 online
    await pool.query(
      "UPDATE souls SET status = 'online', updated_at = NOW() WHERE id = $1",
      [soulId]
    );

    // 创建子进程（fork 自身，进入 --soul 分支）
    const proc = fork(__filename, ['--soul', soulId], {
    ...
    proc.on('exit', (code) => {
      ...
      pool.query(
        "UPDATE souls SET status = 'offline', updated_at = NOW() WHERE id = $1",
        [soulId]
      ).catch(() => {});
    });
   ...
   * @returns AI 回复内容
   */
  async sendMessage(soulId: string, type: 'chat' | 'group', payload: any): Promise<string> {
    if (!this.souls.has(soulId)) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const queued: QueuedMessage = { type, payload, requestId };
  ...
  private dispatch(soulId: string, queued: QueuedMessage): void {
    const proc = this.souls.get(soulId);
    if (!proc) {
  ...
  private processNext(soulId: string): void {
   ...
   * 处理单聊请求（兼容旧接口）
   */
  async handleChat(soulId: string, payload: { messages: any[] }): Promise<string> {
