...
router.get("/health", async (_req, res) => {
    try {
        const result = await db_1.db.query("SELECT 1");
...
router.get("/stats", async (_req, res) => {
    try {
        const souls = await db_1.db.query("SELECT status, COUNT(*) FROM souls GROUP BY status");
        const messages = await db_1.db.query("SELECT COUNT(*) FROM messages WHERE DATE(created_at) = CURRENT_DATE");
        const tokens = await db_1.db.query("SELECT COALESCE(SUM(total_tokens), 0) as total FROM token_usage WHERE DATE(created_at) = CURRENT_DATE");
        const soulList = await db_1.db.query("SELECT id, name, status FROM souls ORDER BY id");
        res.json({
            success: true,
            data: {
                souls: souls.rows,
