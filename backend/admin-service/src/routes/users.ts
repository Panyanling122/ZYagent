...
router.post("/", async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO users (username, password_hash, permissions, bound_soul_id, is_active, created_at, updated_at)
       VALUES ($1,$2,$3,$4,true,NOW(),NOW()) RETURNING id, username, is_admin, is_active, bound_soul_id, permissions, created_at`,
      [username, hash, JSON.stringify(permissions || []), bound_soul_id || null]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
...
router.put("/:id/username", async (req, res) => {
  try {
    ...
    const exists = await db.query("SELECT id FROM users WHERE username = $1 AND id != $2", [username, req.params.id]);
    if (exists.rows.length > 0) {
      res.status(400).json({ success: false, error: "用户名已存在" });
      return;
    }
    await db.query("UPDATE users SET username = $1, updated_at = NOW() WHERE id = $2", [username, req.params.id]);
...
router.put("/:id/soul", async (req, res) => {
  try {
    ...
    await db.query("UPDATE users SET bound_soul_id = $1, updated_at = NOW() WHERE id = $2", [soulId || null, req.params.id]);
...
router.put("/:id/permissions", async (req, res) => {
  try {
    ...
    await db.query("UPDATE users SET permissions = $1, updated_at = NOW() WHERE id = $2",
