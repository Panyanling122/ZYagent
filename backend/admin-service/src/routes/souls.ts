...
router.get("/:id", async (req, res) => {
  try {
    const result = await db.query("SELECT * FROM souls WHERE id = $1", [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ success: false, error: "Soul not found" });
...
router.delete("/:id", async (req, res) => {
  try {
    await db.query("DELETE FROM souls WHERE id = $1", [req.params.id]);
