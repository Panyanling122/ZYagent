...
import { encryptApiKey, isEncrypted } from '../utils/crypto';
...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const result = await db.query(
      'SELECT id, name, base_url, model_map, is_active, created_at, updated_at FROM providers ORDER BY name'
    );
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await db.query(
      'SELECT id, name, base_url, model_map, is_active, created_at, updated_at FROM providers WHERE id = $1 LIMIT 1',
      [id]
    );
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // 加密 API Key
    const encryptedKey = isEncrypted(api_key) ? api_key : encryptApiKey(api_key);

    const result = await db.query(
      `INSERT INTO providers (name, base_url, api_key, model_map, is_active)
       VALUES ($1, $2, $3, $4, false)
       RETURNING id, name, base_url, model_map, is_active, created_at`,
      [name, base_url, encryptedKey, JSON.stringify(model_map || {})]
    );
...
router.post('/:id/activate', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await db.query(
      'UPDATE providers SET is_active = true, updated_at = NOW() WHERE id = $1 RETURNING id, name, is_active',
      [id]
    );
...
router.post('/:id/deactivate', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await db.query(
      'UPDATE providers SET is_active = false, updated_at = NOW() WHERE id = $1 RETURNING id, name, is_active',
      [id]
    );
