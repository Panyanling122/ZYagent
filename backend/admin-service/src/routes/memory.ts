...
router.get("/", async (_req, res) => {
  try {
    const l1 = await db.query("SELECT COUNT(*) as count FROM l1_messages WHERE is_active = true");
    const l2 = await db.query("SELECT COUNT(*) as count FROM l2_summaries");
    const l3 = await db.query("SELECT COUNT(*) as count FROM l3_knowledge");
...
router.put("/l3/:id", async (req, res) => {
  ...
  const result = await db.query(
    "UPDATE l3_knowledge SET content=$1, updated_at=NOW() WHERE id=$2 RETURNING *",
