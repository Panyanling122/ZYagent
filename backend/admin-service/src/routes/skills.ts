...
router.post("/", async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO skills (name, version, description, code, skill_md, depends_on, bound_souls, created_at, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,NOW(),NOW()) RETURNING *`,
      [name, version || "1.0.0", description || "", code || "", skill_md || "", JSON.stringify(depends_on || []), JSON.stringify(bound_souls || [])]
    );
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE skills SET name=$1, version=$2, description=$3, code=$4, skill_md=$5, is_active=$6, updated_at=NOW() WHERE id=$7 RETURNING *`,
      [name, version, description, code, skill_md, is_active, req.params.id]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
